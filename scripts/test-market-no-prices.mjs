...
async function account(role){const auth=await send('/api/mobile/auth',{action:'register',name:'No prices test',email:`price-${role}-${Date.now()}@example.test`,password:'Long-test-password-123',acceptTerms:true});await send('/api/market',{action:'register',name:'Test '+role,role,acceptTerms:true},auth.token);return auth.token;}
const customer=await account('customer'),provider=await account('provider');
const common={title:'No financial fields test',description:'Test service description',category:'Ремонт',city:'Рига'};
...
console.log('PASS: task, profile and bid without price; supplied financial fields ignored and absent in API responses');
