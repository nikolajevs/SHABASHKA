...
for(const [ru,lv,en] of rows){assert.ok(ru&&lv&&en);const slots=s=>[...s.matchAll(/\{(\w+)\}/g)].map(m=>m[1]).sort();assert.deepEqual(slots(lv),slots(ru),ru);assert.deepEqual(slots(en),slots(ru),ru);assert.ok(!/[А-Яа-яЁё]/.test(lv+en),`Untranslated: ${ru}`);}
for(const file of ['app/market.tsx','app/i18n/provider.tsx','app/layout.tsx']){const sf=ts.createSourceFile(file,fs.readFileSync(file,'utf8'),ts.ScriptTarget.Latest,true,ts.ScriptKind.TSX);function visit(n){if(ts.isCallExpression(n)&&['t','translate'].includes(n.expression.getText(sf))){const arg=n.arguments[n.expression.getText(sf)==='translate'?1:0];if(arg&&ts.isStringLiteral(arg))assert.ok(keys.has(arg.text.trim()),`Missing: ${arg.text}`);}if(ts.isJsxText(n))assert.ok(!/[А-Яа-яЁё]/.test(n.text),`Hardcoded JSX in ${file}: ${n.text}`);ts.forEachChild(n,visit);}visit(sf);}
...
console.log(`PASS: ${rows.length} complete translation entries, placeholders, source coverage, locale fallback, server-rendered language and API errors in all three languages.`);
