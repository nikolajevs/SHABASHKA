...
export function AuthLogout(){const{t,locale,errorText}=useLanguage(),[busy,setBusy]=useState(false),[error,setError]=useState('');return <><button type="button" className="auth-link" disabled={busy} onClick={async()=>{setBusy(true);try{await authRequest('logout',{},locale);location.assign('/signout-with-chatgpt?return_to=/');}catch(e){setError(errorText(e instanceof Error?e.message:''));setBusy(false);}}}>{t('Выйти')}</button>{error&&<span role="alert">{error}</span>}</>;}
