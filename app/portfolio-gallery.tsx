...
export function PortfolioGallery({images,previewCount}:{images:string[];previewCount?:number}) {
  const {t}=useLanguage();
  ...
  return <>
    <div className="portfolio-gallery">{images.slice(0,previewCount??images.length).map((src,index)=><button className="portfolio-thumbnail" type="button" key={index} aria-label={t('Увеличить фото {number}',{number:index+1})} onClick={()=>{setSelected(index);setZoom(false);}}><img src={src} alt={`${t('Работа')} ${index+1}`} loading="lazy"/>{previewCount&&index===previewCount-1&&images.length>previewCount?<span className="portfolio-more">+{images.length-previewCount}</span>:null}</button>)}</div>
