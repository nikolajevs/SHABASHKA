...
export async function expirePrivacyMarkers(){
 ...
 await db.batch([
  db.prepare("DELETE FROM records WHERE kind='block' AND parent IN (SELECT id FROM records WHERE kind='account' AND json_extract(data,'$.erased')=1 AND created<?)").bind(cutoff),
  db.prepare("DELETE FROM records WHERE kind='account' AND json_extract(data,'$.erased')=1 AND created<?").bind(cutoff),
 ]);
}
...
export async function eraseAccount(owner: string) {
  ...
  await db.batch([
    ...
    db.prepare("INSERT INTO records(id,kind,owner,parent,data,created) VALUES (?,'account',?,NULL,?,?) ON CONFLICT(id) DO UPDATE SET data=excluded.data,created=excluded.created").bind('account:'+owner,owner,JSON.stringify({erased:true,inactive:true}),new Date().toISOString()),
